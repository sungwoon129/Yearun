...
public class websocket extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		System.out.println("123"+ session.getAttributes());
    	System.out.println("userid값은 "+userId);
    	//2. 들어온 아이디로 어느 방에 있는 지 확인한다.
    	
    	ChatMemberDto userRoom = service.getRoomMember(new ChatMemberDto(userId, "",""));
    	...
    	service.updateRoomMember(new ChatMemberDto(userId, "", priroom));
    	...
    	logger.info("세션추가:"+session.getId()+"접속자아이디:"+userId+" 현재채팅접속자:"+sessionList.size()+"명");
    	
    	//6. 입장 시 해당 방 인원 수를 증가시킨다.
    	service.updateChatCountInc(userRoom.getRoom());
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
    	...
    	//2. 접속을끊을 때 해당 아이디로 DB에서 어느 방에 존재하는지 확인한다.
     	ChatMemberDto member = service.getRoomMember(new ChatMemberDto(userId, "", ""));
    	...
    	service.updateChatCountDec(member.getPriroom());
    	...
    	for(int i=0; i<sessionList.size(); i++) {
    		...
    		//9. 접속자가 속한 방에만 접속했음을 알린다.
    		if(userRoom.getPriroom().equals(roomList.get(sessionList.get(i)))) {
 ...
 //서버가 클라이언트로부터 메시지를 받았을때
    ...
    public String JsonData(String id, Object msg) {
    	Date d = new Date();
    	 SimpleDateFormat time = new SimpleDateFormat("aaahh시mm분");

    	 
    	JsonObject jsonObject = Json.createObjectBuilder().add("message", 
    	
    	"<a href='#none' onclick=\"insertWisper('"+id+"')\"><b>["+id+"]</b></a><br><li class='chat'>&nbsp;&nbsp;"+msg+"</li><span class ='time'>"+time.format(d)+"</span>"+"</br>").build();
    ...
    public String JsonDataOpen(String id) {
    	JsonObject jsonObject = Json.createObjectBuilder().add("message",
    			"<li class='inchat'><a href='#none' onclick=\"insertWisper('"+id+"')\">"+"<b>["+ 
    id+"]</b> 님이 <b style='color:blue'>접속</b>하셨습니다.</a></li>").build();
    	StringWriter write = new StringWriter();
    ...
    public String JsonDataClose(String id) {
    	JsonObject jsonObject = Json.createObjectBuilder().add("message","<li class='inchat'><b>["+ id+"]</b> 님이 <b style='color:red'>퇴장</b>하셨습니다.</li>").build();
    	StringWriter write = new StringWriter();
    ...
    public String JsonWisper(String fromId, Object msg) {
    	...
    	fromId+"]</b>님의 귓속말</a><br><li class='chat'>"+msg+"</li>"+"<span class ='time'>"+time.format(d)+"</span><br>").build();
    	StringWriter write = new StringWriter();
    ...
    public String getRoomName() throws Exception{
    	...
    	List<ChatDto> roomList =  service.getRoomList();
    ...
    public String getRoomName(String name) throws Exception{
    	...
    	List<ChatDto> roomList =  service.searchRoomList(name);
