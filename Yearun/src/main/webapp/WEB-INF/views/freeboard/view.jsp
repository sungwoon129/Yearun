...
<title>여운_여운과 함께 당신의 플랜을 계획해보세요</title>
 <jsp:include page="../gnb/GNB.jsp" flush="true" />
<script>
$(document).ready(function() {
	var freeCode = ${item.freeCode};
	var id = '${sessionScope.user}';
	var recommendImage = $('.recommendChange');
	
	$.ajax({
		url: '/freeboard/checkRecommend',
		type: 'POST',
		data: {'freeCode':freeCode,'id':id},
...
function deleteBtn(){
    ...
    if(result == true){
        location.href = 'delete?freeCode=${item.freeCode}';
    }else{
    return false;
    }
}
...
<div class="currentaddress">
 	...
 	<div class="contentsArea">
	   <h2 class="title">자유게시판</h2>
		...
		<div class="line_one"><span class="freeTitle">${item.freeTitle}</span> <span class="temper">자유게시판</span> <span class="regDate"><fmt:formatDate
		...
		<div class="freeContents">${item.freeContext}</div>

		
		<div class="attach">
			<span class="attachText">첨부파일:</span> <span><a href="${pageContext.request.contextPath}/upload/${item.freePhotoA}" target="_blank">${item.freePhotoA}</a></span>
		</div>
	 	<script>
...
<div class="container">
        ...
        <form name="commentInsertForm">
            <div class="input-group">
               ...
               <input type="text" class="form-control" id="content" name="content" placeholder="로그인 후 이용하실 수 있습니다."   onkeydown="enterkey()" disabled="disabled">
...
<c:choose>
		<c:when test="${sessionScope.user == item.id or sessionScope.user == 'admin'}">
			<button class="update"><a href="update?freeCode=${item.freeCode}"><img src="${pageContext.request.contextPath}/resources/images/update.jpg"></a></button>	
			<button class="delete" onclick="deleteBtn()"><%-- <a href="delete?freeCode=${item.freeCode}"> --%><img src="${pageContext.request.contextPath}/resources/images/delete.jpg"><!-- </a> --></button>
		</c:when>
		<c:otherwise>
...
<button class="reply"><a href="replyadd?originNo=${item.originNo}&groupOrd=${item.groupOrd}&groupLayer=${item.groupLayer}"><img src="${pageContext.request.contextPath}/resources/images/reply.jpg"></a></button>
...
window.onload = function(){
	var freeCode = ${item.freeCode};
	 commentList(freeCode);
}


var freeCode = ${item.freeCode} //게시글 번호
...
function commentInsert(insertData){
    ...
    $.ajax({
        ...
        success : function(){
                ...
                commentList(freeCode); //댓글 작성 후 댓글 목록 reload
                $('[name=content]').val('');
...
function commentUpdate(cno, content){
    ...
    a += '<input type="text" class="form-control" onkeydown="updateEnterkey('+cno+')" id="content" name="content_'+cno+'" value="'+content+'"/>';
...
function updateEnterkey(cno){
	   if(window.event.keyCode ==13){
		  
		   commentUpdateProc(cno); //Insert 함수호출(아래)
	   }
	}  
...
function commentUpdateProc(cno){
    ...
    $.ajax({
        ...
        success : function(data){
            if(data == 1) commentList(freeCode); //댓글 수정후 목록 출력 
        }
    });
}
...
function commentDelete(cno){
    $.ajax({
        ...
        type : 'post',
        success : function(data){
            if(data == 1) commentList(freeCode); //댓글 삭제후 목록 출력 
        }
    });
}
