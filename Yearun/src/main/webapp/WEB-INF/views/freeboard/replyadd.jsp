...
<!-- action : 에디터에 입력한 html 코드를 전달받을 Controller페이지 URL -->
	...
	<form action="replyadd" method="post" name="form1" enctype="multipart/form-data">
		<input type="hidden" name="originNo" value="${item.originNo}">
		<input type="hidden" name="groupOrd" value="${item.groupOrd}">
		<input type="hidden" name="groupLayer" value="${item.groupLayer}">
		<input type="hidden" id="thum" value="default" name="thumbnail">
		<div class="freeTitle">
			<label>제목 &nbsp;&nbsp;</label> <input type="text" name="freeTitle" placeholder="게시글 제목을 입력해주세요." style="width:620px" onkeyup="chkword(this,40)">
	
		</div>
		<textarea name="freeContext" id="summernote"></textarea>
...
//                 $('#summernote').summernote("insertNode", img_name[0]);
		...
		<div class="alarm-box"><p class="alarm">첨부파일은 최대 1개, 40mb까지 등록이 가능하며 <br>파일 형식은 jpg,gif,psd,png,tif,zop,ms office,아래한글(hwp),pdf만 가능합니다.</div>
	</div>
