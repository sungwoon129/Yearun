...
var reviewTitle = document.form1.reviewTitle.value;
var reviewContext = document.form1.reviewContext.value;
...
<div class="currentaddress">
 ...
 <div class="allContents">
	...
	<input type="hidden" name="thumbnail" value="default" id="thum">
	

		<div class="freeTitle">
			<label>제목 &nbsp;&nbsp;</label> <input type="text" name="reviewTitle" placeholder="게시글 제목을 입력해주세요." style="width:620px" value="${item.reviewTitle}" onkeyup="chkword(this,40)">
	
		</div>
		<textarea name="reviewContext" id="summernote" placeholder="이미지만 업로드시 이미지만 업로드하지마시고 텍스트도 함께 추가해주세요.">${item.reviewContext}</textarea>
...
//                 $('#summernote').summernote("insertNode", img_name[0]);
			...
			<label for="input_file">파일 선택</label> 
                            <input class="upload-name" readonly="readonly" style="width: 200px;" value="${item.reviewPhotoA}">

                            
                          <input type="file" id="input_file" class="upload-hidden" name="attach" > 
		...
		<div class="alarm-box"><p class="alarm">첨부파일은 최대 20mb까지 등록이 가능하며 <br>파일 형식은 jpg,gif,psd,png,tif,zop,ms office,아래한글(hwp),pdf,excel,zip,7z만 가능합니다.</div>
	</div>
